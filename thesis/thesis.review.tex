...
I analyze the vibrational solvatochromism of guanine modes. Finally,
...
References: MeCN in gas\hyp{}phase (Refs.X), MeCN in CCl$_4$, THF, MeOH, H$_2$O, CF$_3$CH$_2$OH and 
(CF$_3$)$_2$CHOH (Ref.X), MeCN in CH$_3$(CH$_2$)$_2$OH (Ref.X), MeSCN in gas-phase (Ref.X), MeSCN in 
CCl$_4$, CHCl$_3$, CH$_2$Cl$_2$, MeCN, EtOH, MeOH, H$_2$O, CF$_3$CH$_2$OH and (CF$_3$)$_2$CHOH (Ref.X). In the 
...
here to first read the Appendix~\ref{a:vibrational-analysis} 
...
vibrational energy states that give non\hyp{}zero integral, i.e., $n_J \pm 1$ 
...
T_j = \frac{1}{M_i\omega_i^2}\fderiv{U}{Q_i}  \Bigg|_{{\bf Q}_{0_A}}       
 \equiv -\delta Q_j
\end{equation}
and
...
obtain the expansion coefficiets from Eq.\eqref{e:Q-space-transform}.
%
...
It was shown by MacDowell and Buckingham that the first\hyp{}order approximation works 
very well for simple, spatially localized high\hyp{}frequency modes such as
Li--H or H--X stretches where X is halogen atom.\citep{McDowell.Buckingham.SpectrChimActaA.2005,
McDowell.Buckingham.JACS.2005,McDowell.Buckingham.MolPhys.2005}
The theory is very accurate also for polyatomic molecules
as domonstrated by Herrebout~\emph{et al.} who studied
...
'2' (Eq.~\eqref{e:dQ-approx-T1-expl-lead}). For simplicity, we have used Cho's coarse\hyp{}grained
...
the Hessian matrix, iii) we diagonalized the Hessian matrix and extracted the frequencies.
As model systems, we considered fully optimized structures of $N$-methylacetamide (NMA) 
and MeN$_3$ 
...
that were computed for the examplary clusters by explicit diagonalization
of full solute's Hessian matrixes according to Eq.~\eqref{e:force-const-cho} 
and electrostatic
...
\hline\hline
\multicolumn{1}{c}{$\delta {\bf Q}^{[1]}$} & 
\multicolumn{1}{c}{$\delta {\bf Q}^{[2]}$} \\
\hline
...
are distributed on heavy atoms and amide proton hence the number of centers is 6. See Chapter~\ref{c:my-model}, 
...
CN, NC or N$_3$.\citep{Blasiak.Ritchie.Webb.Cho.XXX.2016,Maj.Ahn.Blasiak.Kwak.Han.Cho.XXX.2016}
...
using Eq.~\eqref{e:sdd} one has to obtain integral derivative wrt atomic position
...
\end{subequations}
%
where the summations extend over the canonical molecular orbitals (CMO's\footnote{not localized
...
\end{equation}
%
and $y$ denotes the atomic center.
Here, we shall apploximate $\mathcal{V}^{B}\approx \sum_{y\in B} q_y \mathcal{T}^{(y)}$
...
\section{Modeling the solute's far environment\label{s:far-environ}}
...
\subsection{Distributed muptipole models: SolDMA, SolCAMM and SolLMTP\label{s:solx-electrost}}
...
with electric fields  will be analyzed later in Chapter~\ref{c:cn-modes}.
...
Blasiak.Ritchie.Webb.Cho.XXX.2016,Maj.Ahn.Blasiak.Kwak.Han.Cho.XXX.2016}
 %
...
where we analyze ring deformation modes
...
\begin{equation} \label{e:coe}
 {\bf R}_{\rm COE}^{(j)} \equiv \sum_m \left\{ (v^{(j)}_{m;x})^2 + (v^{(j)}_{m;y})^2 + (v^{(j)}_{m;z})^2\right\} {\bf r}_m
\end{equation}
%
where $m$ denotes atomic site and $v^{(j)}_{m;x}$ is the $x$th
...
and their sum is labeled as "TOT".
...
from Ref.~\citep{Lee.Choi.Cho.JCP.2012} (labeled as "Fitting" in this Figure).
...
We analyzed twelve configurations taken every 1000
...
different sites of free RalGDS protein (\emph{upper}) amd RalGDS that is non\hyp{}covalently bound to Ras'
(\emph{lower}). 
...
of varying polartity. Moreover, it was shown that the extinction coefficient
...
analyzed by Rey and Hynes~\citep{Rey.Hynes.JCP.1998} 
...
the distributed site multipoles~\citep{Waegele.Gai.JPCL.2010,
...
Peng.Jones.Tokmakoff.JACS.2011} Therefore, modeling the
...
by analyzing the atomic vibration amplitude maps shown
...
%Here, we have to specify atoms of sulfonate groups and carbon atom in the
%backbone (just to properly place this fragment in space such that it is 
%superimposed with sulfonate group in target system). 
...
centered at molecular origin.\citep{Lee.Choi.Cho.JCP.2012}
...
evaluated with GAMESS program by utilizing MAKEFP routine.\citep{GAMESS.JCC.1993}
Pipek\hyp{}Mezey localization~\citep{Pipek.Mezey.JCP.1989}
...
%
\begin{equation}
 \int_0^\infty {\rm d}\Omega f(\Omega) \cong \sum_{n=1}^{12} w_n \frac{2v\nu_0}{(1-t_n)^2}f_n
\end{equation}
%
where $v_0=0.30$ and $t_n$ and $w_n$ are the
absicae and weights of the quadrature (spanning at the
...
\paragraph{Numerical differentiation of various quantities that are related to localized
...
transforms the canonical molecular orbitals (CMO) into localized molecular orbitals (LMO)
...
the resulting ai vectors could be different from the vectors in ${\bf U}_{\rm ref}$. 
Thus, to restore the one\hyp{}to\hyp{}one
correspondence, one can analyze the following similarity matrix
%
...
were obtained by utilizing {\sc Antechamber} package~\citep{Antechamber.JMGM.2006}
...
26 $\times$ 29 $\times$ 23 Bohrs with the origin at the center of geometry of NMA molecule (orientation of the
...
Geometry optimizations and vibrational analyses of model QM clusters of MeCN and
...
localize molecular orbitals including core electrons. Distributed polarizabilities centered
at localized molecular orbitals (LMO) were computed by using {\sc GAMESS} package.
Geometry optimization and harmonic vibrational analyses were performed by using
...
CCl$_4$ were performed by using {\sc Amber11} program, Parm99 force field was utilized to
describe the solvent whereas MeSCN was modeled by PM3 semi\hyp{}empirical
...
30000 steps of steepest descent energy minimization were performed and the cascade
...
were analyzed by using SolEFP theory) were saved every 0.10~ps. Throughout all these
...
trajectories was performed by using the rigid molecule algorithm42 that utilizes the
...
In brief, 2D umbrella sampling strategy was utilized to sample the SCN
...
MeSCN in bulk solvents from Section~\ref{s:dw-cn-bulk} was modeled with PM3 Hamiltonian~\citep{Stewart.JCC.1988}.
...
Geometry optimizations and vibrational analyses of $N$-methylguanine
...
was utilized for all the production runs. Very tight criteria
