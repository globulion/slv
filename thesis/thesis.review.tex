...
where
%
\begin{equation} \label{e:wca-X-matrix}
 A_{ij} = 
\left\{\begin{matrix}
0 &\text{for $i=j$}\\ 
...
solute's Hessian matrix in more detail. However, there potentially exists a possibility
of reducing the strength of mode mixing by unilizating the \emph{isotopic effec}.
It will be interesting to examine similarity matrices between modes 0$-$4 in cases when atoms in guanine
rings are isotopically labeled. If it is possible to make the difference between the gas\hyp{}phase
vibrational frequencies of ring modes larger, the extent of mode mixing
could be smaller according to Eq.~\eqref{e:wca-X-matrix}.
