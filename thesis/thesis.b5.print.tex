\documentclass[b5paper,twoside,fleqn,11pt,draft]{book}
...
through strongly polar non\hyp{}protic
...
interactions which were relatively large. Note that the latter cannot be correlated with electrostatic potential
...
role in the spectral line shapes and positions and the comprehensive and unified
vibrational solvatochromism theory is lacking. Apparently,
...
vibrational solvatochromism of many IR probes. Our goal is to shed new light
on the importance of non\hyp{}Coulombic effects
in the vibrational solvatochromism, as well as develop an all\hyp{}encompassing model
that could be used to study complex systems such as macromolecules under biological conditions.
...
\subsection{Charge Penetration Effects}

Our explicit models of all electrostatic contributions to the
vibrational frequency shift (i.e., Coulombic, induction and dispersion)
were so far based on the distributed multipole expansions. However,
it turns out that it is not sufficient to describe correctly
the intermolecular interactions even if the multipole expansions 
converge well. This is due to the fact that molecular charge densities
are not finite and are spread in space leading to \emph{charge penetration}.
This purely quantum mechanical effect influences the electrostatic potential operators and has to be included
in accurate modelling when molecules are close to each other. 
In particular, interactions between molecules in which at least one of them is charged
can lead to quite large charge penetration effects. If such effects were not
properly included the, so called, `polarization catastrophe` could happen leading
to unphysically large attraction forces. In practical molecular dynamics calculations, 
this would first cause implosion and then explosion of the system. It would also cause
unphysically large induction frequency red shifts. One of such examples is shown
in Table.

Charge penetration effect on the Coulombic frequency shift 
can be easily estimated from first principles.
It was shown that the Coulombic charge penetration energy can be separately computed
from the following formula:
%
\begin{equation}
 U^{\rm Pen} = \sum_{a\in A}\sum_{b\in B} 
 U_{ab}^{\rm Pen}
\end{equation}
%
where
%
\begin{equation}
 U_{ab}^{\rm Pen} = -2\sqrt{\frac{-1}{2\ln{\vert S_{ab}\vert}}} \times \frac{S_{ab}^2}{r_{ab}}
\end{equation}
%
Thus, we can find that the associated frequency shift contribution reads
%
\begin{equation}
 \Delta\omega_j^{\rm Pen} \approx -\frac{1}{M_j\omega_j} 
                          \sum_i \frac{g_{ijj}}{M_i\omega_i^2} 
                                 \sum_{ab} \left\{
                                      \frac{S_{ab}}{r_{ab}} \fderiv{S_{ab}}{Q_i}
                                      \left[ X_{ab} - \frac{2}{X_{ab}}\right]
                                     +\left( \frac{S_{ab}}{r_{ab}} \right)^2 X_{ab}^{-1} \fderiv{r_{a}}{Q_i}
                                 \right\}
\end{equation}
%
with
%
\begin{equation}
 X_{ab} \equiv \sqrt{-2\ln{\vert S_{ab} \vert}}
\end{equation}
%
Hence, to compute this contribution one needs the overlap integrals, the LMO centroids
and their derivatives, which are also necessary for the exchange\hyp{}repulsion
and the charge\hyp{}transfer frequency shifts calculations. 

Unfortunately, the first\hyp{}principles damping of induction energy
is not well understood nor developed. There have been semi\hyp{}empirical
damping formulae used, as in Tang and Toennies, for instance, that are based
on the overlap... However,
the empirical nature of such approaches makes the application of the first principles
model of the vibrational solvatochromism rather tricky because the derivatives
of the fitting damping parameters with respect to the normal modes are not uniquely defined.
Nevertheless, it would be interesting to develop a model that includes induction (and dispersion) charge penetration
effects and is fairly transferable between different types of environments and ions.
Since most of the systems studied in this Work are neutral, we put this 
for further development and will not be discussed in this Thesis in detail.
...
\begin{equation} \label{e:dw-soleds-hf}
 \Delta\omega_j^{\rm SolEDS\sslash HF} = 
...
\begin{equation} \label{e:dw-soleds-mp2}
 \Delta\omega_j^{\rm SolEDS\sslash MP2} = 
              \Delta \omega_{j;{\rm el}}^{(10)}    + 
              \Delta \omega_{j;{\rm ex}}^{\rm HL}  +
              \Delta \omega_{j;{\rm del}}^{\rm HF} +
...
part of $\Delta\omega_j^{\rm SolEDS\sslash MP2}$ quantitatively differs from
$\Delta\omega_j^{\rm SolEDS\sslash HF}$ obtained at the HF level of theory,
...
polarization term $E^{(nm)}_{\rm pol}$ ($\mathscr{A}=\mathbb{I}$)
...
effects by using SolEDS$\sslash$MP2 method. The orientation\hyp{}
...
SolEFP$\sslash$HF/6-311++G** method. The repulsion interaction induces amide I
...
SolEFP$\sslash$HF/6-311++G** method. Water molecules occupy the H-bonding sites
...
phase state obtained by SolEFP/6-311++G**$\sslash$Parm99 method. In this
Table, we also show full width at half maximum (FWHM) of vibrational
frequency shift static distributions obtained from SolEFP$\sslash$Parm99 calculations.
All values in cm$^{-1}$.
...
\multirow{5}{*}{Gas phase} & 1731$^a$ & \multirow{5}{*}{$\Delta\omega^{\rm Expt.}$} 
                                      & \multirow{5}{*}{$\Delta\omega^{\rm SolEFP/sslash Parm99}$} 
                                      & \multirow{5}{*}{Error} & \\
                           & 1714$^a$ & &&&\\
                           & 1731$^b$ & &&&\\
                           & 1713$^b$ & &&&\\
...
the SolEFP$\sslash$HF/6-311++G** method with molecular dynamics 
...
SolEDS$\sslash$HF/6-311++G** calculations predict
...
\caption{
SolEFP$\sslash$HF/6-311++G** frequency shift components. (a) The SolEFP components contributing to the CN
...
SolEDS$\sslash$MP2/6-311++G** method. Bond lengths are in Bohr and frequency shifts in
cm$^{-1}$.
...
that were studied by using SolEDS$\sslash$MP2/6-311++G** method.
...
present work. Nonetheless, our SolEDS$\sslash$HF frequency shifts are
...
SolEFP, those analysis results can be compared to SolEDS$\sslash$HF
...
%
\begin{table}[t!]
\caption{SolEDS$\sslash$HF/6-311++G** frequency shifts of the CN/NC stretch mode with
...
\caption{
Molecular structures of the model systems used for SolEDS$\sslash$HF/6-311++G**
...
SolEFP$\sslash$CMD calculations are run by script {\tt slv{\textunderscore}md} which reads the
special input file.
...
\section{Semi-Empirical Determination of Vibrational Solvatochromic Charges}

In one of the variants of ESF approach, the vibrational frequency shift is modelled as~\citep{Lee.Choi.Cho.JCP.2012}
%
\begin{equation} \label{e:esf-charges}
 \Delta \omega_j \approx \sum_x l^{(j)}_x \phi({\bf r}_x)
\end{equation}
%
where $l^{(j)}_x$ is the solvatochromic charge associated with the $j$th vibrational
mode and the $x$th distributed (generally non\hyp{}atomic) site. Since the dependence
of the vibrational frequency on the local environment is far more complicated than 
shown in Eq.~\eqref{e:esf-charges}, the solvatochromic charges have to be fit onto a carefully
chosen set of points located at ${\bf r}_x$ to some
benchmark set of molecular clusters in order to achieve satisfying accuracy.
