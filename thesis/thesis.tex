...
U^{\rm Ind} = - \frac{1}{2} {\bf p} \cdot {\bf p} = - \frac{1}{2} {\bf F}^T \cdot {\bf D}^{-1} \cdot {\bf F}
\end{equation}
%
...
\begin{subequations}
 \begin{align}
   {\bf P}^{(i)} &= \fderiv{{\bf R}_A}{Q_i} \\
   G^{(i)}       &= {\bf R}_{AB} \cdot \fderiv{{\bf R}_A}{Q_i}
 \end{align}
\end{subequations}
%
...
If the total molecular (single\hyp{}centered) multipole 
expansions of the charge distributions of the molecules 
$A$ and $B$ are used, both the lowest\hyp{}order 
dipole\hyp{}dipole (proportional to $R^{-6}_{AB}$) 
and higher\hyp{}order terms in Eq.~\eqref{e:dw-disp-6} are usually large 
so that a serious convergence problem arises when Eq.~\eqref{e:dw-disp-6} 
is calculated for large solute and solvent molecules with 
spatially delocalized charge distributions. Therefore, 
the distributed site approach has been used in the EFP2 
method\citep{Adamovic.Gordon.MolPhys.2005} to improve 
this convergence of the multipolar expansion series 
for the dispersion interaction energy. Naturally, 
the theory on the dispersion interaction\hyp{}induced 
vibrational solvatochromism can be extended by following 
the same idea. The expressions for the frequency shifts 
and the associated vibrational solvatochromic coefficients 
that we implemented and quantitatively evaluated in 
this work are given as
...
Following Jensen and Gordon\citep{Jensen.Gordon.JCP.1998,Jensen.Gordon.MolPhys.1996} 
the exchange\hyp{}repulsion interaction energy 
