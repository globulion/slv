...
\begin{equation} \label{e:approx-1}
 \Delta \omega \approx -\frac{g_{jjj}}{2M_j^2\omega_j^3} \fderiv{U}{Q_j} 
...
\begin{equation} \label{e:approx-2}
 \Delta \omega \approx -\frac{1}{2M_j\omega_j} \sum_i \frac{g_{ijj}}{M_i\omega_i^2}\fderiv{U}{Q_i} 
\end{equation}
%
...
\begin{equation} \label{e:approx-3}
 \Delta \omega \approx -\frac{1}{2M_j\omega_j} \left[
 \sderiv{U}{Q_j} 
...
So far we were elaborating on the accuracy and computational
...
shown in Fig.XXX. The frequency
shifts evaluated by using (i) constrained\hyp{}solute SolEDS and
SolEFP methods and (ii) analytical SolEFP equations applied
for the fully optimized clusters are summarized in TableXXX.
The
...
contributions. As can be seen in TableXXX, most of
calculated total frequency shifts including both MA and EA
terms are in reasonable quantitative agreement with the full
quantum chemistry calculation results, with exceptions of
constrained-solute SolEFP result for system A and analytical
SolEFP result for system C -- note that, in these two cases,
the resultant frequency shifts are slightly underestimated. The
quantitative agreement found in these comparative investigations
indicates that the frequency shift formula, Eq. (2), on
which all the numerical calculation results presented in this
work are based, is sufficiently accurate and reproduces frequency
shifts with possible errors of about 5--10 cm$^{-1}$, which
...
\subsubsection{Constrained cluster method}
...
Eqs.~\eqref{e:soleds}, \eqref{e:soleds-hf} and \eqref{e:soleds-mp2}. 
