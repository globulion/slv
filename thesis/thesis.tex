...
surrounding molecules,%12-17
which is in notable contrast to the
solvatochromism of CO stretch modes.%1, 18, 19 
By the analysis
...
because, unlike in MeCN case%50, 
...
To explain the molecular origins of such greatly enhanced sensitivity to H-bonding 
...
The structure and basic idea of {\sc Solvshift} (SLV) is shown
...
%}
\caption{
Structure of {\sc Solvshift}. It imports the following 
{\sc Coulomb} modules: {\tt MULTIP}, {\tt QMFile}, {\tt Ints}, 
{\tt Freq}, {\tt Atoms} and {\tt UNITS}. 
...
Figure~\ref{f:slv-solefp} presents the overall procedure 
of SolX methods implemented in the program. 
%
\begin{figure}[t!]
\centering
\setlength\fboxsep{0.4pt}
\setlength\fboxrule{0.5pt}
%\fbox{
\includegraphics[width=0.98\linewidth]{slv-fig-2.eps}
%}
\caption{
\emph{Left:} Evaluation of vibrational property by using SolX methods.
\emph{Right:} Applying SolX models to molecular systems.
\label{f:slv-solefp}}
\end{figure}
%
The principle of operation depends on the method chosen:
%
\begin{itemize}
  \item Run by using {\bf Constrained cluster algorithm (CCA)}:
        ...
        methods (Section~XXX). The vibrational derivatives are evaluated numerically.
...
to be utilized. In the latter case, the solvatochromic model is constructed 
...
These parameters serve as templates to be superimposed later onto
the target system. 

In Table~\ref{t:slv-rma-parameters}
we present the list of parameters that
build IBM's. In the case of solvent molecule,
IBM is equivalent to the conventional EFP2 parameters.
...
need to be computed. The amount of data per one IBM is 
substantial as can be seen in Table~\ref{t:slv-rma-parameters}.
However, the present implementation handles all of these parameters
efficiently and the net computer usage regarding handling IBM's 
...
IBM's are handled by {\tt Frag} class, instances of which are passed to
...
%}
\caption{
Examplary input file for SLV/MD routine.
\label{f:slv-inp-example}}
\end{figure}
...
HF/6-311++G** method%51, 52 
as implemented in the Gaussian09 suite of programs.%53 
Cubic 
...
molecules were computed by the variational\hyp{}perturbational method%45-48 
implemented by G{\'o}ra%54 
in the GAMESS package%55. 
...
undertaken by using MP2/6-311++G** method.%51, 52 
The atomic charges were computed by 
using ChelpG method%56 
