...
which gives 1964 cm$^{-1}$. However, Hessians diagonalization gives correct frequency (1662 cm$^{-1}$) 
only if '2' approximation
is employed.

The similar analysis for MeN$_3$ molecule brings however different results with SolCAMM-4 model
used (see Fig.~\ref{f.NMA.dq}a-d). It can be seen that
the structural distortion is not properly described since the Hessian contain apparently
off-diagonal elements, and even '2' approximation works worse in this case. 
%
\begin{figure}[ht]
\centering
\setlength\fboxsep{0.4pt}
...
\end{figure}
%
The differences in the vibrational solvatochromisms of NMA and MeN$_3$ are quite significant. 
This is because amide I modes are surprisingly well described by purely Coulombic models
which is in stark contrast to triple\hyp{}bonded IR probes such as CN or N$_3$.
We will
discuss it in more detail later when the non-Coulombic effects will also be derived and quantified.
